...
var EdgeFlowDag = (function (_super) {
    ...
    EdgeFlowDag.prototype.render = function () {
        ...
        return (React.createElement(edge_flow_1.EdgeFlow, __assign({}, props), nodes.map(function (node) { return React.createElement(edge_flow_1.Node, __assign({ key: node.id, x: posNodes[node.id].x, y: posNodes[node.id].y }, node), groupedEdges[node.id] && groupedEdges[node.id].map(function (edge) {
            var ee = edgeDict[node.id + "-" + edge.linkTo];
            return React.createElement(edge_flow_1.Edge, __assign({ key: edge.fromForceNode + "-" + edge.linkTo }, edge, { source: ee.p1, p2: ee.p2, p3: ee.p3, target: ee.p4 }));
        })); })));
    };
    return EdgeFlowDag;
}(React.Component));
