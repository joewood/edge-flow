...
var edge_flow_dag_children_1 = require("./edge-flow-dag-children");
exports.EdgeDag = edge_flow_dag_children_1.EdgeDag;
exports.NodeDag = edge_flow_dag_children_1.NodeDag;
var dagre_helper_1 = require("./dagre-helper");
var common_1 = require("./common");
var EdgeFlowDag = /** @class */ (function (_super) {
    ...
    EdgeFlowDag.prototype.render = function () {
        ...
        var allEdges = nodes.reduce(function (p, node) { return p.concat(common_1.getChildrenProps(node.children).map(function (edge) { return (__assign({ fromForceNode: node.id }, edge)); })); }, []);
        var groupedEdges = lodash_1.groupBy(allEdges, function (e) { return e.fromForceNode; });
        return (React.createElement(edge_flow_1.EdgeFlow, __assign({}, props), nodes.map(function (node) { return (React.createElement(edge_flow_1.Node, __assign({ key: node.id, center: posNodes[node.id] }, node), groupedEdges[node.id] &&
            groupedEdges[node.id].filter(function (edge) { return nodeDict[edge.linkTo]; }).map(function (edge) {
