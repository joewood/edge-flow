...
var EdgeFlowDag = (function (_super) {
    ...
    EdgeFlowDag.prototype.getStateFromProps = function (newProps) {
        var graph = dagre_helper_1.getGraphFromNodes(newProps.children);
        return dagre_helper_1.getLayout(graph, newProps.children, newProps.style.width, newProps.style.height);
    };
    ...
    EdgeFlowDag.prototype.render = function () {
        ...
        var nodes = getChildrenProps(children) || [];
        ...
        var round = function (r) { return ({ x: Math.round(r.x * 100) / 100, y: Math.round(r.y * 100) / 100 }); };
        return (React.createElement(edge_flow_1.EdgeFlow, __assign({}, props), nodes
            .map(function (node) { return (React.createElement(edge_flow_1.Node, __assign({ key: node.id, x: Math.round(posNodes[node.id].x * 100) / 100, y: Math.round(posNodes[node.id].y * 100) / 100 }, node), groupedEdges[node.id] && groupedEdges[node.id].map(function (edge) {
            var ee = edgeDict[node.id + "-" + edge.linkTo];
            return React.createElement(edge_flow_1.Edge, __assign({ key: edge.fromForceNode + "-" + edge.linkTo }, edge, { source: round(ee.p1), p2: round(ee.p2), p3: round(ee.p3), target: round(ee.p4) }));
        }))); })));
    };
    return EdgeFlowDag;
}(React.PureComponent));
