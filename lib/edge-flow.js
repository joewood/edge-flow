    ...
    EdgeFlow.prototype.render = function () {
        ...
        var svgLineFn = function (style) {
            return allEdges.map(function (edge) {
                ...
                var styleP2X = style[compKey(edge, "p2") + "X"];
                var styleP2Y = style[compKey(edge, "p2") + "Y"];
                var styleP3X = style[compKey(edge, "p3") + "X"];
                var styleP3Y = style[compKey(edge, "p3") + "Y"];
                var styletoX = style[compKey(edge, "to") + "X"];
                var styletoY = style[compKey(edge, "to") + "Y"];
                if (!styleX || !styleY || !styletoX || !styletoY)
                    throw "Invalid Style";
                return React.createElement("path", { key: edge.from.id + "-" + edge.linkTo, d: "M" + styleX * diagramWidth + " " + styleY * diagramHeight + " C " + styleP2X * diagramWidth + " " + styleP2Y * diagramHeight + " " + styleP3X * diagramWidth + " " + styleP3Y * diagramHeight + " " + styletoX * diagramWidth + " " + styletoY * diagramHeight, stroke: edge.pathColor || defaulltStrokeColor, opacity: edge.pathOpacity || 0.1, fill: "transparent", strokeWidth: edge.pathWidth || 12 });
            });
        };
