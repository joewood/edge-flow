...
var react_motion_2 = require("react-motion");
var defaults = {
    iconStyle: {
        fontSize: 10,
...
var EdgeFlow = (function (_super) {
    ...
    EdgeFlow.prototype.render = function () {
        ...
        var _a = this.props, animate = _a.animate, style = _a.style, onClickNode = _a.onClickNode, selectedNodeId = _a.selectedNodeId, iconStyle = _a.iconStyle, labelStyle = _a.labelStyle, particleStyle = _a.particleStyle, pathStyle = _a.pathStyle;
        var backgroundColor = style.backgroundColor, width = style.width, height = style.height;
        if (!width || isNaN(width) || !height || isNaN(height)) {
            throw "Invalid Height/Width";
        }
        var defaultPathStyle = __assign({}, defaults.pathStyle, { color: Color(backgroundColor).lighten(10).toString() });
        ...
        var springStyles = allEdges.map(function (edge) { return animation_style_1.createEdgeStyle(edge, nodeDict, scale); }).concat(nodes.map(function (node) { return animation_style_1.createNodeStyle(node, node.center, scale); }), [
            {
                key: "transform",
                style: {
                    x: react_motion_2.spring(scale.getTransform().x),
                    y: react_motion_2.spring(scale.getTransform().y),
                    scaleX: react_motion_2.spring(scale.getTransform().scaleX),
                    scaleY: react_motion_2.spring(scale.getTransform().scaleY),
                },
                data: { isNode: false, isEdge: false }
            }
        ]);
        return (React.createElement("div", { key: "root", style: composedStyle },
            React.createElement("svg", { key: "svg", width: width, height: height, style: { left: 0, top: 0, backgroundColor: backgroundColor, position: "absolute" }, onClick: function () { return onClickNode && onClickNode({ nodeId: null, graph: null, screen: null }); } },
                ...
                React.createElement(react_motion_1.TransitionMotion, { key: "svg-anim", defaultStyles: defaultStyles, styles: springStyles }, function (styles) {
                    return React.createElement("g", { key: "g", transform: "scale(" + styles[styles.length - 1].style.scaleX + "," + styles[styles.length - 1].style.scaleY + ") \n                                translate(" + styles[styles.length - 1].style["x"] + "," + styles[styles.length - 1].style["y"] + ")" },
            ...
            React.createElement("div", { key: "particleContainer", style: { pointerEvents: "none", position: "absolute", left: 0, top: 0 } },
                React.createElement(react_motion_1.TransitionMotion, { key: "motion-anim", defaultStyles: defaultStyles, styles: springStyles }, function (styles) {
                    return React.createElement(particle_canvas_1.ParticleCanvas, { key: "particles", width: diagramWidth, height: diagramHeight, run: animate, backgroundColor: backgroundColor }, animation_style_1.isEdgeStyles(styles).map(function (edgeStyle) {
                        return React.createElement(particle_canvas_1.ParticleEdge, __assign({ key: edgeStyle.data.from.id + "-" + edgeStyle.data.linkTo }, edgeStyle.data, { particleStyle: __assign({}, defaults.particleStyle, particleStyle, edgeStyle.data.particleStyle, { size: scale.avgSizeToScreen(edgeStyle.data.particleStyle && edgeStyle.data.particleStyle.size || 10) }), pathStyle: __assign({}, defaults.pathStyle, pathStyle, edgeStyle.data.pathStyle), p0: {
