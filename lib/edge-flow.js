...
var EdgeFlow = (function (_super) {
    ...
    EdgeFlow.prototype.render = function () {
        ...
        var scaleX = function (x) { return Math.round(((x - min.x) + (max.x - min.x) * 0.08) / ((max.x - min.x) * 1.16) * diagramWidth * 10) / 10; };
        var scaleY = function (y) { return Math.round(((y - min.y) + (max.y - min.y) * 0.08) / ((max.y - min.y) * 1.16) * diagramHeight * 10) / 10; };
        ...
        var svgLineFn = function (styles) {
            return styles
                .filter(function (style) { return !!style.data.ratePerSecond; })
                .map(function (edgeStyle) {
                var d = edgeStyle.style;
                var edge = edgeStyle.data;
                return React.createElement("path", { key: edgeStyle.key, d: "M" + d.fromx + "," + d.fromy + " C " + d.p2x + "," + d.p2y + " " + d.p3x + "," + d.p3y + " " + d.tox + "," + d.toy, stroke: edge.pathColor || defaulltStrokeColor, opacity: edge.pathOpacity || 0.1, fill: "transparent", strokeWidth: edge.pathWidth || 12 });
            });
        };
        var edgeStyle = function (edge, useSpring) {
            if (useSpring === void 0) { useSpring = false; }
            var from = edge.source || edge.from;
            var to = edge.target || nodeDict[edge.linkTo];
            var p2 = edge.p2 || from;
            var p3 = edge.p3 || to;
            return {
                key: compKey(edge),
        ...
        var nodeStyle = function (node, point) {
            return ({
                key: node.id,
        ...
        var defNodeStyle = function (node, point) {
            return ({
                key: node.id,
        ...
        var defaultStyles = allEdges.map(function (edge) { return edgeStyle(edge); }).concat(nodes.map(function (node) { return defNodeStyle(node, { x: node.x, y: node.y }); }));
        var springStyles = allEdges.map(function (edge) { return edgeStyle(edge, true); }).concat(nodes.map(function (node) {
            return nodeStyle(node, { x: node.x, y: node.y });
        }));
            ...
            React.createElement("svg", { key: "svg", width: width, height: height, style: { left: 0, top: 0, backgroundColor: backgroundColor, position: "absolute" }, onClick: function () { return onClickNode({ nodeId: null, graph: null, screen: null }); } },
                React.createElement(react_motion_1.TransitionMotion, { key: "svg-anim", defaultStyles: defaultStyles, styles: springStyles }, function (styles) {
                    return React.createElement("g", { key: "g" }, svgLineFn(styles).concat(styles
                        ...
                        .filter(function (style) { return !style.data.ratePerSecond && !style.data.group && !style.data.annotation; })
                        .map(function (nodeStyle) {
            ...
            React.createElement("div", { key: "particleContainer", style: { pointerEvents: "none", position: "absolute", left: 0, top: 0 } },
                React.createElement(react_motion_1.TransitionMotion, { key: "motion-anim", defaultStyles: defaultStyles, styles: springStyles }, function (styles) {
                    return React.createElement(particle_canvas_1.ParticleCanvas, { key: "particles", width: diagramWidth, height: diagramHeight, run: run, backgroundColor: backgroundColor }, styles
                        .filter(function (edge) { return edge.data.ratePerSecond; })
                        .map(function (edgeStyle) {
