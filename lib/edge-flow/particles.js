    ...
    Particles.prototype.updateBuffers = function (edges, width, height) {
        try {
            ...
            for (var _a = 0, edges_2 = edges; _a < edges_2.length; _a++) {
                var edge = edges_2[_a];
                var variationMin = (edge.variationMin === undefined) ? -0.01 : edge.variationMin;
                var variationMax = (edge.variationMax === undefined) ? 0.01 : edge.variationMax;
                // set-up vertices in edgedata
                this.textureData.setVec2(VERTEX_ROW, edgeIndex, convertBezierPoints({ x: edge.fromX, y: edge.fromY }, { x: edge.fromX, y: edge.fromY }), convertBezierPoints({ x: edge.toX, y: edge.toY }, { x: edge.toX, y: edge.toY }));
                ...
                // bezier
                this.textureData.setVec2(BEZIER_ROW, edgeIndex, convertBezierPoints(edge.p2, edge.p2), //{ x: edge.fromX, y: edge.fromY }),
