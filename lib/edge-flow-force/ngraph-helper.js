...
function getLayout(g, childrenNodes, width, height) {
    ...
    var rect = forceLayout.getGraphRect();
    var newNodes = mapNodes(g).map(function (node) { return getPosition(forceLayout, node, rect, 20, width, height); });
    return newNodes;
}
...
function getPosition(layout, n, rect, blockWidth, screenWidth, screenHeight, center) {
    ...
    var width = rect.x2 - rect.x1 + blockWidth;
    var height = rect.y2 - rect.y1 + blockWidth;
    var scaleX = screenWidth / width;
    var scaleY = screenHeight / height;
    var offsetX = rect.x1 * -1 + blockWidth * 0.5;
    var offsetY = rect.y1 * -1 + blockWidth * 0.5;
    var pos = layout.getNodePosition(n.id);
    return {
        id: n.id,
